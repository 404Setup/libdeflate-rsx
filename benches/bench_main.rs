...
fn bench_decompress_offset17_tail(c: &mut Criterion) {
    let size = 64;
    let pattern = b"12345678901234567";
    let mut original_data = Vec::with_capacity(size);
    while original_data.len() < size {
    ...
    let mut group = c.benchmark_group("Decompress offset17 Tail");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs offset17 tail", &size, |b, &_size| {
        let mut decompressor = Decompressor::new();
        b.iter(|| {
            decompressor
                .decompress_deflate_into(&compressed_data[..compressed_size], &mut out_buf)
...
fn bench_compress_level10_large(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // Repeating pattern to ensure many matches, triggering the DP logic heavily
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz";
    let mut data = Vec::with_capacity(size);
    while data.len() < size {
    ...
    let mut group = c.benchmark_group("Compress Level 10 Large");
    group.throughput(Throughput::Bytes(size as u64));

    group.bench_with_input("libdeflate-rs level 10", &size, |b, &_size| {
        let mut compressor = Compressor::new(10).unwrap();
        let bound = compressor.deflate_compress_bound(size);
        let mut out_buf = vec![0u8; bound];
        b.iter(|| {
            compressor
...
fn bench_compress_level6_large(c: &mut Criterion) {
    let size = 1024 * 1024; // 1MB
    // Repeating pattern to ensure many matches, triggering the greedy match logic
    let pattern = b"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz";
    let mut data = Vec::with_capacity(size);
    while data.len() < size {
        data.extend_from_slice(pattern);
    ...
    let mut group = c.benchmark_group("Compress Level 6 Large");
    group.throughput(Throughput::Bytes(size as u64));
...
criterion_group!(
    benches,
    bench_compress_level10_large,
    bench_compress_level6_large,
    bench_crc32_large,
    ...
    bench_decompress_offset17_tail,
