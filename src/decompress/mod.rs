            ...
            unsafe {
                ...
                if offset >= length {
                    ...
                    if offset == 1 || offset == 2 || offset == 4 {
                        ...
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat0); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat1); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat2); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat3); copied += 8; }
                        while copied < length {
                        ...
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat2); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat3); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat4); copied += 8; }
                        if copied + 8 <= length { std::ptr::write_unaligned(dest_ptr.add(copied) as *mut u64, pat5); copied += 8; }
                        while copied < length {
                            *dest_ptr.add(copied) = *src_ptr.add(copied);
                            copied += 1;
                        }
